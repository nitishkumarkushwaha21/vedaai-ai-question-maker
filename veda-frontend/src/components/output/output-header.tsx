...
export function OutputHeader({ title, subtitle, ctaLabel = "Download as PDF", onDownload }: OutputHeaderProps) {
  return (
    <div className="rounded-2xl border border-slate-700 bg-[#1b1d22] p-4 text-white shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <button
          ...
          aria-label={ctaLabel}
          disabled={!onDownload}
          className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-semibold text-slate-900 disabled:cursor-not-allowed disabled:opacity-60 md:text-sm"
        >
